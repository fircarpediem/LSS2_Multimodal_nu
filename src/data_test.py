...
from src.datasplit import create_splits_scenes
...
class NuscData(torch.utils.data.Dataset):
    ...
    def get_scenes(self):
        ...
        split = {
            'v1.0-trainval': {True: 'train', False: 'test'},
    ...
    def get_txt(self, rec, dataroot):
        action_root = os.path.join(dataroot, 'action_all')
        action_name = rec['token'] + '.json'
        action_path = os.path.join(action_root, action_name)
        with open(action_path) as jfile:
            action = json.load(jfile)
        action_tensor = torch.FloatTensor(action)

        desc_root = os.path.join(dataroot, 'desc_all')
        desc_name = rec['token'] + '.json'
        desc_path = os.path.join(desc_root, desc_name)
        with open(desc_path) as jfile:
            desc = json.load(jfile)
        desc_tensor = torch.FloatTensor(desc)
        # print(action_tensor)
        # print(desc_tensor)
        return action_tensor, desc_tensor
...
def compile_data(version, dataroot, data_aug_conf, grid_conf, bsz,
    ...
    traindata = parser(nusc, is_train=True, data_aug_conf=data_aug_conf,
                         grid_conf=grid_conf, data_root=os.path.join(dataroot, version))
...
def compile_data_test(version, dataroot, data_aug_conf, grid_conf, bsz,
                 nworkers, parser_name):
    nusc = NuScenes(version='v1.0-{}'.format(version),
                    dataroot=os.path.join(dataroot, version),
                    verbose=False)
    ...
    testdata = parser(nusc, is_train=False, data_aug_conf=data_aug_conf,
                       grid_conf=grid_conf, data_root=os.path.join(dataroot, version))
